...
class Templates:
    ...
    def get_template(self, uilang: str, lang: str, template_name: str) -> str:
        # Приоритет отдается специфичному шаблону для конкретного языка (например russian/french/...)
        if uilang in self._templates and lang in self._templates[uilang] and template_name in self._templates[uilang][lang]:
            return self._templates[uilang][lang][template_name]
        if uilang in self._templates and template_name in self._templates[uilang]:
            return self._templates[uilang][template_name]
        raise KeyError(f'Template "{template_name}" not found for uilang="{uilang}", lang="{lang}".')