...
class WordsProgressDB:
    def __init__(self, db_path):
        ...
        self.progress_df['last_review_date'] = pd.to_datetime(self.progress_df['last_review_date']).astype('datetime64[ns]')
    ...
    def get_word_progress(self, chat_id, word_id) -> Optional[Item]:
        ...
        if row.shape[0] != 1:
            ...
            raise ValueError('Number of rows satisfying the condition must be exactly 1.')
        r = row.iloc[0]
        item = Item(e_factor=float(r['e_factor']), num_reps=float(r['num_reps']),
                    next_review_date=r['next_review_date'], last_review_date=r['last_review_date'],
                    last_interval=int(r['last_interval']), word_id=word_id)
        self._lock.release()
        return item
